...
|---|---|---|
| `R-01` | Use `ecv1/{device}/camera-adapter/cmd/sb/{verb}` and select the camera with body `instance` for this delivery. Command addressing follows core decision D-U28 (optional-instance UNS addressing): the camera adapter is single-instance, so commands are component-scope (no instance token) and the camera is chosen by the body `instance` field, not by a per-instance command topic. | D-CAM-18 and §§12.1, 13, 26, 27 |
