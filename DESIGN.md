...
- Direct RTSP-URL cameras through a dedicated `rtsp` backend (no ONVIF): still-image capture from a
  configured `rtsp://`/`rtsps://` stream, reusing the ONVIF backend's RTSP engine and network/credential
  trust surface. No PTZ, snapshot, or discovery (a raw URL exposes none).
