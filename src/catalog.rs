//! Durable SQLite catalog for capture jobs, command idempotency, and recovery.
//!
...
/// Terminal state plus the exact schema-v1 terminal body, committed in one transaction.
///
/// The body is the durable record of the capture's outcome: it answers `sb/capture-status`, it is
/// what the sidecar contains, and it is what the volatile terminal announcement carries. Nothing
/// about message DELIVERY is durable -- there is no retained envelope and no delivery state.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub struct PendingInstall {
    ...
    /// Exact success terminal write to reuse during recovery.
...
pub enum TerminalOutcome {
    /// This caller won and committed the terminal state. It -- and only it -- announces the result.
    Won(JobRecord),
...
impl Catalog {
    ...
    ///
    /// Each member announces its own terminal result. This transaction retains the group command
    /// reply in both the group row and its idempotency ledger.
    ...
    /// Commits a non-cancellation terminal state. Exactly one caller wins, and only that caller
    /// may announce the result.
    ...
    ///
    /// This method deliberately returns stored material only. Recovery must build the terminal body
    /// elsewhere and pass it to [`Catalog::interrupt_recovered`].
    ...
    /// Prunes terminal jobs past the result-retention window.
    /// Non-terminal jobs and OUTCOME_UNKNOWN ledgers are never selected.
    ...
    /// Prunes a terminal group, its member jobs, and its ledger once every member is terminal.
    ...
    ///
    /// Non-terminal jobs, group members, and OUTCOME_UNKNOWN operations remain ineligible even
    /// while the maximum is exceeded.
...
fn migrate(connection: &mut Connection) -> Result<()> {
    ...
    match version {
        ...
        4 => transaction.execute_batch(MIGRATION_V5)?,
        _ => {
            return Err(CameraError::Catalog(format!(
...
/// Retires the durable outbox.
///
/// The terminal announcement is volatile now: it is published once, after the terminal is durable,
/// and never retained or retried. So the `outbox` table goes -- with any rows still in it, which is
/// exactly the trade being taken: an announcement that had not yet been delivered when this
/// migration runs is lost, while the capture it describes remains on disk and SUCCEEDED in the
/// catalog. `pending_success` keeps only what recovery actually needs, the staged terminal body;
/// its envelope columns cannot be dropped in place (they are UNIQUE/CHECK-constrained), so the
/// table is rebuilt and its staged rows carried across -- an in-flight install must still be able
/// to finish after an upgrade.
const MIGRATION_V5: &str = r#"
DROP INDEX IF EXISTS outbox_pending_idx;
DROP TABLE IF EXISTS outbox;

CREATE TABLE pending_success_v5 (
    capture_id TEXT PRIMARY KEY,
    terminal_result_json TEXT NOT NULL CHECK(json_valid(terminal_result_json)),
    terminal_at_ms INTEGER NOT NULL,
    FOREIGN KEY(capture_id) REFERENCES jobs(capture_id) ON DELETE CASCADE
) STRICT;
INSERT INTO pending_success_v5(capture_id,terminal_result_json,terminal_at_ms)
    SELECT capture_id,terminal_result_json,terminal_at_ms FROM pending_success;
DROP TABLE pending_success;
ALTER TABLE pending_success_v5 RENAME TO pending_success;
"#;
...
/// Validates the terminal write's own consistency: its state, its retained error, and its body.
///
/// The body checks used to be made against the encoded envelope the outbox carried. The envelope is
/// gone, but the checks are not optional -- the body is now the ONE artifact the announcement is
/// rebuilt from, so a body that does not say what it is cannot be committed.
fn validate_terminal_write(write: &TerminalWrite) -> Result<()> {
    ...
    let body = terminal_body(write)?;
    if body.get("schemaVersion").and_then(Value::as_u64) != Some(1) {
        return Err(CameraError::Catalog(
            "terminal result schemaVersion must be 1".to_owned(),
        ));
    }
    for field in ["eventId", "captureId", "cameraId", "correlationId"] {
        match body.get(field).and_then(Value::as_str) {
            Some(value) if !value.is_empty() => {}
            _ => {
                return Err(CameraError::Catalog(format!(
                    "terminal result {field} must be a non-empty string"
                )));
            }
        }
    }
...
fn validate_terminal_for_job(write: &TerminalWrite, job: &JobRecord) -> Result<()> {
    ...
    {
        return Err(CameraError::Catalog(
            "terminal result captureId/cameraId does not match the durable job".to_owned(),
        ));
    }
    if let Some(origin) = &job.origin_correlation_id {
        if body.get("correlationId").and_then(Value::as_str) != Some(origin.as_str()) {
            return Err(CameraError::Catalog(
                "terminal command result correlation does not match the creating request"
                    .to_owned(),
            ));
        }
    }
    match (
        ...
        _ => {
            return Err(CameraError::Catalog(
                "terminal result group identity does not match the durable job".to_owned(),
            ));
        }
    }
    Ok(())
...
fn terminal_body(write: &TerminalWrite) -> Result<&serde_json::Map<String, Value>> {
    write.result.as_object().ok_or_else(|| {
        CameraError::Catalog("terminal result must be a JSON object".to_owned())
    })
}
...
fn insert_pending_success(
    ...
    transaction.execute(
        "INSERT INTO pending_success (capture_id,terminal_result_json,terminal_at_ms) \
         VALUES (?1,?2,?3)",
        params![
            capture_id,
            serde_json::to_string(&write.result)?,
...
const JOB_SELECT: &str = "SELECT capture_id,instance,verb,request_id,state,canonical_request_json,request_hash,\
 ...
 (SELECT terminal_at_ms FROM pending_success p WHERE p.capture_id=jobs.capture_id) FROM jobs";
...
mod tests {
    ...
    fn terminal_write_for(
        ...
        let mut result = json!({
            "schemaVersion": 1,
            "eventId": format!("terminal-event-{sequence}"),
            "captureId": capture_id,
            "cameraId": instance,
            "correlationId": format!("corr-{capture_id}"),
            "winner": sequence,
            "state": job_state_token(state),
        });
    ...
    async fn open_verifies_schema_pragmas_and_rejects_a_second_owner() {
        ...
        assert!(
            !tables.contains("outbox"),
            "a fresh catalog must not create the retired outbox table"
        );
    ...
    #[tokio::test]
    async fn waiter_and_recovery_lifecycles_keep_durable_boundaries() {
        let directory = TempDir::new().unwrap();
        let catalog = open(&directory).await;
        let job = direct_job("recovery-waiter", "recovery-waiter");
        ...
        let waiter = WaiterRecord {
            ...
            capture_id: "recovery-waiter".to_string(),
        ...
        assert!(catalog.add_waiter(waiter.clone()).await.unwrap());
        assert_eq!(
            catalog.waiters("recovery-waiter", 1_999).await.unwrap(),
            vec![waiter]
        );
        assert!(
            catalog
                .waiters("recovery-waiter", 2_000)
                .await
                .unwrap()
                .is_empty()
        );
        ...
        assert!(matches!(
            catalog.queue_job("recovery-waiter", 1_100).await.unwrap(),
            StateCasOutcome::Changed(_)
        ));
        ...
        assert!(matches!(
            catalog
                .interrupt_recovered(
                    "recovery-waiter",
                    vec![JobState::Queued],
                    terminal_write("recovery-waiter", JobState::Interrupted, 9),
                )
                .await
                .unwrap(),
        ...
        assert!(
            catalog
                .interrupt_recovered(
                    ...
                    terminal_write("recovery-waiter", JobState::Interrupted, 10),
                )
                .await
                .is_err()
        );
        let terminal = catalog.job("recovery-waiter").await.unwrap().unwrap();
        assert_eq!(terminal.state, JobState::Interrupted);
        assert_eq!(
            terminal.terminal_result.as_ref().unwrap()["eventId"],
            json!("terminal-event-9"),
            "the terminal body the first interruption won is the durable one"
        );
    }

    /// Thirty-two racing writers, one durable terminal body -- and exactly one winner to announce it.
    #[tokio::test]
    async fn concurrent_terminal_writers_commit_one_state_and_one_exact_body() {
        let directory = TempDir::new().unwrap();
        let catalog = open(&directory).await;
        catalog
        ...
        for task in tasks {
            let (_sequence, outcome) = task.await.unwrap();
            match outcome {
                TerminalOutcome::Won(record) => {
                    winners += 1;
                    assert_eq!(record.terminal_result.as_ref(), Some(&exact_write.result));
                }
        ...
        assert_eq!(
            winners, 1,
            "exactly one writer may win, because exactly one may announce"
        );
        let terminal = catalog.job("terminal-race").await.unwrap().unwrap();
        assert_eq!(terminal.terminal_result.as_ref(), Some(&exact_write.result));
    ...
    async fn cancellation_and_installation_use_one_transactionally_ordered_cas() {
        ...
        let terminal_count = catalog
            .execute(|connection| {
                Ok(connection.query_row(
                    "SELECT COUNT(*) FROM jobs WHERE terminal_at_ms IS NOT NULL",
                    [],
                    |row| row.get::<_, i64>(0),
                )? as usize)
            })
    ...
    /// The v1-era `outbox` and `pending_success` tables, exactly as they were before v5 retired them.
    ///
    /// A migration is only worth anything against the schema it actually has to migrate, so the old
    /// shape is written out here rather than approximated.
    const LEGACY_OUTBOX_SCHEMA: &str = r#"
CREATE TABLE outbox (
...
"#;
    ...
    /// Builds a database as it stood at `version`, with the tables that version owned.
    fn legacy_catalog(path: &Path, version: i64) -> Connection {
        let connection = Connection::open(path).unwrap();
        connection
            .execute_batch("CREATE TABLE jobs(capture_id TEXT PRIMARY KEY) STRICT;")
            .unwrap();
        connection.execute_batch(LEGACY_OUTBOX_SCHEMA).unwrap();
        if version >= 2 {
    ...
    fn every_supported_schema_version_migrates_to_the_current_one() {
        for start in 1..SCHEMA_VERSION {
            ...
            let mut connection = legacy_catalog(&path, start);
            ...
            assert!(
                table_exists(&connection, "group_schedule_cursors"),
                "v{start} did not gain group_schedule_cursors"
            );
            assert!(
                !table_exists(&connection, "outbox"),
                "v{start} did not drop the retired outbox table"
            );
        }
    }

    /// The v5 migration DROPS the `outbox` table on a catalog created at an older version.
    ///
    /// This pins the trade the removal takes, in both directions. The outbox table and everything
    /// still queued in it go -- an announcement that had not been delivered when the component was
    /// upgraded is lost, permanently, and that is the accepted cost. What must NOT go is the durable
    /// truth: the staged `pending_success` body of an install that was in flight across the upgrade
    /// survives, because without it that capture could never be recovered and finished.
    #[test]
    fn the_v5_migration_drops_the_outbox_table_and_keeps_the_staged_terminal_body() {
        let directory = TempDir::new().unwrap();
        let path = directory.path().join("v4.sqlite3");
        let mut connection = legacy_catalog(&path, 4);
        connection
            .execute(
                "INSERT INTO jobs(capture_id) VALUES ('cap-upgraded')",
                params![],
            )
            .unwrap();
        connection
            .execute(
                "INSERT INTO outbox \
                 (event_key,capture_id,message_kind,topic,envelope_uuid,encoded_envelope,\
                  created_at_ms,available_at_ms) \
                 VALUES ('evt-1','cap-upgraded','terminal','ecv1/d/c/i/app/image/captured',\
                  'uuid-1',X'0102',1000,1000)",
                params![],
            )
            .unwrap();
        connection
            .execute(
                "INSERT INTO pending_success \
                 (capture_id,terminal_result_json,terminal_at_ms,event_key,message_kind,\
                  header_name,topic,envelope_uuid,encoded_envelope,created_at_ms,available_at_ms) \
                 VALUES ('cap-upgraded','{\"eventId\":\"evt-1\"}',1000,'evt-1','terminal',\
                  'ImageCaptured','ecv1/d/c/i/app/image/captured','uuid-1',X'0102',1000,1000)",
                params![],
            )
            .unwrap();
        ...
        let version: i64 = connection
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .unwrap();
        assert_eq!(version, 5, "the migration must advance the schema to v5");
        assert!(
            !table_exists(&connection, "outbox"),
            "the v5 migration must drop the outbox table, undelivered rows and all"
        );
        assert!(
            connection
                .query_row(
                    "SELECT 1 FROM sqlite_master WHERE type='index' AND name='outbox_pending_idx'",
                    [],
                    |row| row.get::<_, i64>(0),
                )
                .optional()
                .unwrap()
                .is_none(),
            "the outbox index must go with its table"
        );

        let pending_columns = columns(&connection, "pending_success");
        assert_eq!(
            pending_columns,
            vec![
                "capture_id".to_string(),
                "terminal_result_json".to_string(),
                "terminal_at_ms".to_string()
            ],
            "pending_success must keep only what recovery needs: the staged terminal body"
        );
        let staged: String = connection
            .query_row(
                "SELECT terminal_result_json FROM pending_success WHERE capture_id='cap-upgraded'",
                [],
                |row| row.get(0),
            )
            .expect("an install staged before the upgrade must still be recoverable after it");
        assert_eq!(staged, "{\"eventId\":\"evt-1\"}");
    }

    /// A v5 catalog opens, and a capture committed through it never touches an outbox.
    #[tokio::test]
    async fn a_migrated_catalog_serves_captures_at_the_current_schema() {
        let directory = TempDir::new().unwrap();
        let path = directory.path().join(DATABASE_NAME);
        let mut legacy = legacy_catalog(&path, 4);
        migrate(&mut legacy).unwrap();
        drop(legacy);

        let catalog = open(&directory).await;
        assert_eq!(
            catalog.health().await.unwrap().schema_version,
            SCHEMA_VERSION
        );
        catalog
            .accept_job(direct_job("cap-after-upgrade", "cap-after-upgrade"))
            .await
            .unwrap();
        catalog.queue_job("cap-after-upgrade", 1_100).await.unwrap();
        assert!(matches!(
            catalog
                .commit_terminal(
                    "cap-after-upgrade",
                    terminal_write("cap-after-upgrade", JobState::Failed, 60)
                )
                .await
                .unwrap(),
            TerminalOutcome::Won(_)
        ));
        let terminal = catalog.job("cap-after-upgrade").await.unwrap().unwrap();
    ...
    /// A recovered interruption is durable across a restart, and is not offered for recovery twice.
    #[tokio::test]
    async fn a_recovered_interruption_survives_a_restart_and_is_not_recovered_again() {
        let directory = TempDir::new().unwrap();
        ...
        let terminal = reopened.job("crash-accepted").await.unwrap().unwrap();
        assert_eq!(terminal.state, JobState::Interrupted);
        assert_eq!(terminal.terminal_result.as_ref(), Some(&exact.result));
        assert!(reopened.recovery_jobs().await.unwrap().is_empty());
    }

    /// Retention reclaims a terminal job only once it is past the result-retention window.
    ///
    /// Nothing else holds a terminal record back any more. It used to be held by its own undelivered
    /// message; there are no messages left to be undelivered.
    #[tokio::test]
    async fn retention_prunes_terminal_jobs_only_once_they_are_past_the_window() {
        let directory = TempDir::new().unwrap();
        let catalog = open(&directory).await;
        for (capture, request, sequence) in [("older", "older", 81), ("newer", "newer", 82)] {
            catalog
        ...
        // `terminal_write` stamps terminal_at_ms = 20_000 + sequence: 20_081 and 20_082.
        assert_eq!(catalog.prune_terminal_jobs(20_082, 10).await.unwrap(), 1);
        assert!(catalog.job("older").await.unwrap().is_none());
        assert!(catalog.job("newer").await.unwrap().is_some());
        assert_eq!(catalog.prune_terminal_jobs(20_082, 10).await.unwrap(), 0);
        assert_eq!(catalog.prune_terminal_jobs(40_000, 10).await.unwrap(), 1);
        assert!(catalog.job("newer").await.unwrap().is_none());
    }
